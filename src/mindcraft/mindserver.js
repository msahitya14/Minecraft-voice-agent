...
export function createMindServer(host_public = false, port = 8080) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('send-message', (agentName, data) => {
            // Normalize different client emit shapes.
            // Some clients (python-socketio) may send a single array [agentName, data]
            // or an object {agent: agentName, data: ...} — handle those cases.
            let target = agentName;
            let payload = data;
            try {
                if (Array.isArray(agentName) && agentName.length >= 2) {
                    target = agentName[0];
                    payload = agentName[1];
                } else if (agentName && typeof agentName === 'object') {
                    if (agentName.agent) {
                        target = agentName.agent;
                        payload = agentName.data || agentName.payload || payload;
                    }
                }
            } catch (e) {
            ...
            if (!agent_connections[target]) {
                console.warn(`Agent ${target} not in game, cannot send message via MindServer.`);
                return;
            }
            try {
                agent_connections[target].socket.emit('send-message', payload)
            } catch (error) {
